...
namespace web.page.project
{
    public partial class projectAdd : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (projectNO == null) //访问页面，非提交表单
            {
                //动态生成业主单位下拉表
                //employerSel.InnerHtml = "<option value=\"\">请选择业主单位</option><option value=\"in\" >未找到，手动输入</option>";
                employerSel.Items.Add(new ListItem("请选择业主单位", ""));
                employerSel.Items.Add(new ListItem("未找到，手动输入", "in"));
                DataSet emDs = QuaryUser("SELECT EMPLOYER FROM PRO_EMPLOYER");
                for (int i = 0; i < emDs.Tables[0].Rows.Count; i++)
                {
                    //employerSel.InnerHtml += "<option>" + emDs.Tables[0].Rows[i]["EMPLOYER"].ToString() + "</option>";
                    employerSel.Items.Add(new ListItem(emDs.Tables[0].Rows[i]["EMPLOYER"].ToString()));
                }
                return;
            }
            ...
            //查看是否是新的业主单位，是则存入数据库
            DataSet theemDs = QuaryUser("SELECT EMPLOYER FROM PRO_EMPLOYER WHERE EMPLOYER = '" + projectEmployer + "'");
            if (theemDs.Tables[0].Rows.Count == 0)
            {
                OperateUser("INSERT INTO PRO_EMPLOYER VALUES ('" + projectEmployer + "')");
            }
            //LayuiMsg("创建项目成功");
        ...
        protected Int32 SaveProject(string prono, string proname, string prodistrict, string proriver, string prolocation, string proemployer, string userid)
        {
            ...
            //创建项目资料文件夹
            string projectFolder = @"E:\\Ourgis_ProjectFiles\\Project_" + newSysId.ToString() + "\\\\";
