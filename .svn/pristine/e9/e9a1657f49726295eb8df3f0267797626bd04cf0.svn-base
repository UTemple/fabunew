layui.config({
    ...
    //var perdetail_index;
    //window.permissionDetail = function (obj) {
    //    var id = obj.getAttribute("value");
    //    var infostr = './userPermissionDetail.aspx?pa=' + id;
    //    perdetail_index = layui.layer.open({
    //        title: "权限管理",
    //        type: 2,
    //        content: infostr,
    //        end: function () {
    ...
    //    })
    //    $(window).resize(function () {
    //        layui.layer.full(perdetail_index);
    //    })
    //    layui.layer.full(perdetail_index);
    //}
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
                for (var i = 0; i < currData.length; i++){
                    ...
                    + '<td><a href="./userPermissionDetail.aspx?pa=' + currData[i].ID + '" style="color:darkred;font-style:italic">' + currData[i].PERMISSION + '</a></td>'
                    //+ '<td><a href="javascript:;" value="' + currData[i].ID + '" onclick="permissionDetail(this);" style="color:darkred;font-style:italic">' + currData[i].PERMISSION + '</a></td>'
