...
//
    ...
    //经纬度定位功能
    var XCoo=0,YCoo=0;
    $("#gopos-button").click(function () {
        var lat = $("#inputLatitude").val()//纬度
        var lon = $("#inputLongitude").val()//经度
        var postdata = {"lat":lat,"lon":lon};
        //var postdata = {"str1":lat,"str2":lon};
        $.ajax({   
              type: "Post",
              //url: "maptest.aspx/TestFunc",
              url: "../coordinateconvert/WGStoGZJSService.aspx/GWStoGZServer",   
              //方法传参的写法一定要对，str为形参的名字,str2为第二个形参的名字   
              data: JSON.stringify(postdata),   
              contentType: "application/json; charset=utf-8",   
              dataType: "json",   
              success: function(data) {   
                //返回的数据用data.d获取内容,格式为“X,Y”  
                //alert(data.d);
                var tmparr = data.d.split(',');
                XCoo = tmparr[0];
                YCoo = tmparr[1];
                locateCoordinatePoint(XCoo,YCoo);
              },   
              error: function(err) {   
                alert(err);   
              }   
    ...
    function onConvertXSucceeded(result) {//调用后台方法时的回调，返回转换后的数据
        XCoo = result;
    }
